...
curl -s -X POST http://localhost:3000/contacts \
  ...
  -d '{"lastName": "山田", "firstName": "太郎", "email": "yamada@example.com", "phone": "090-1234-5678", "message": "詳細を教えてください"}' | jq
```
...
| HTTP Status | 説明 |
|-------------|------|
| 400 | バリデーションエラー（空の姓/名、不正なメールアドレス、不正な ID など） |
...
curl -s -X POST http://localhost:3000/contacts \
  -H "Content-Type: application/json" \
  -d '{"lastName": "Test", "firstName": "User", "email": "bad", "message": "Msg"}' | jq
